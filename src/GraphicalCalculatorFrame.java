...
public class GraphicalCalculatorFrame extends JFrame
{
	...
	private final class GraphicalCalculatorPanel extends JPanel implements MouseListener
	{
		...
		private void setupRegions()
		{
            ...
            {
                regions[i] = new Rectangle(REGION_START_X + (i*REGION_INC_X),REGION_START_Y,REGION_WIDTH,REGION_WIDTH);
            }
		}
		...
		public boolean setSelectedRegionContents(String content)
		{
			...
			if(selectedRegion == 0 ||  selectedRegion == 2 || selectedRegion == 4) {
			    ...
			    try {
			        operand = Integer.parseInt(content);
			    }
			    ...
			    if( operand >=  0 && operand <= 9) {
			        ...
			        switch(selectedRegion) {
		                case 0:  operands[0] = Integer.parseInt(content);
		                    break;
		                case 2:  operands[1] = Integer.parseInt(content);
		                    break;
		                case 4:  operands[2] = Integer.parseInt(content);
		                    break;
			        }
			    }
			    else {
			...
			if(selectedRegion == 1 ||  selectedRegion == 3) {
                ...
                if(content == "+" || content == "-" || content == "*") {
                    ...
                    switch(selectedRegion) {
                        case 1:  operators[0] = content;
                            break;
                        case 3:  operators[1] = content;
                            break;
                    }
                }
                else {
    ...
    public GraphicalCalculatorFrame() {
        ...
        setOperand.addActionListener((e) -> {
            ...
            if (operandEntry.getText().length() == 1) {
                gcPanel.setSelectedRegionContents(operandEntry.getText());
                errorMessage.setText("");
            }
            else if(gcPanel.selectedRegion == 1 || gcPanel.selectedRegion == 3) {
        ...
        setOperator.addActionListener((e) -> {
            ...
            if(gcPanel.setSelectedRegionContents(operand)) {
                errorMessage.setText("");
            }
            else if (gcPanel.selectedRegion == 0 || gcPanel.selectedRegion == 2 || gcPanel.selectedRegion == 4) {
